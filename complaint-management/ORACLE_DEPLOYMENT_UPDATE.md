...
```powershell
cd "d:\Coding\Web Dev\CoRe_Test\CoRe_Test\complaint-management\frontend"
npm run build
```
...
# Create .env file with database credentials (only needed first time)
cat > .env << 'EOF'
...
## Notes
- **Frontend is built locally** and pushed to GitHub, then deployed from the build/ directory
- Backend now uses .env file for database credentials (must be created on server first time only)
- Frontend .env files point to Oracle Cloud (http://140.245.253.253/api)
